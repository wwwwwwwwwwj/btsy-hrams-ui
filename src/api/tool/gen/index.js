 ...
 * 分页查询代码生成
 */
export async function pageGen(params) {
 ...
 * 批量删除代码生成
 */
export async function removeGen(ids) {
 ...
 * 分页查询可导入的表
 */
export async function pageGenDb(params) {
              ...
              children: [
                {
                  ...
                  meta: {
                    language: 'vue',
                    text: templateEngine(
                      tplCategory === 'tree' ? editTreeTemplate : editTemplate,
                      templateData
                    ),
 ...
 * 获取数据源名称
 */
export async function listDataName() {
  const res = await request.get('/tool/gen/getDataNames');
  if (res.data.code === 200) {
    return res.data.data;
  }
  return Promise.reject(new Error(res.data.msg));
