<template>
  <ele-page class="workplace-page">
    ...
    <ele-card bordered :body-style="{ padding: 0 }" style="overflow: hidden">
      <div class="workplace-bottom">
        <el-link
          type="primary"
          icon="IconProPlusCircleOutlined"
          underline="never"
    ...
    <!-- 添加卡片弹窗 -->
    <ele-modal :width="720" v-model="visible" title="未添加的卡片">
  ...
  const handleCommand = (command, item) => {
    switch (command) {
      ...
      case 'edit': // 编辑
        ...
        break;
      case 'remove': // 删除
        ElMessageBox.confirm(`确定要删除“${item.title}”卡片吗?`, '系统提示', {
          type: 'warning',
          draggable: true
        })
          .then(() => {
            data.value = data.value.filter((d) => d.name !== item.name);
            cacheData();
          })
          .catch(() => {});
        break;
  ...
  .workplace-page {
    ...
    :deep(.el-col) {
      &.sortable-chosen > .ele-card {
        box-shadow: var(--ele-card-hover-shadow);
      }
