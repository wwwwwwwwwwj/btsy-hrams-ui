<!-- 快捷方式 -->
<template>
  <ele-sortable-row
    v-model="data"
    :row-props="{ gutter: 16 }"
    :draggable-props="{ itemKey: 'id', onUpdate: cacheData }"
  >
    <template #item="{ item }">
      <ele-card bordered shadow="hover" :body-style="{ padding: 0 }">
        <router-link :to="item.url" class="app-link">
          <el-icon class="app-link-icon" :style="{ color: item.color }">
            <component :is="item.icon" />
          </el-icon>
          <div class="app-link-title">{{ item.title }}</div>
        </router-link>
      </ele-card>
    </template>
  </ele-sortable-row>
</template>

<script setup>
  import { ref } from 'vue';

  const CACHE_KEY = 'workplace-links';

  /** 默认顺序 */
  const DEFAULT = [
    {
      id: 1,
      icon: 'IconProUserOutlined',
      title: '用户',
      url: '/system/user',
      colProps: { md: 3, sm: 6, xs: 12 }
    },
    {
      id: 2,
      icon: 'IconProAnalysisOutlined',
      title: '分析',
      url: '/monitor/cache',
      color: '#95de64',
      colProps: { md: 3, sm: 6, xs: 12 }
    },
    {
      id: 3,
      icon: 'IconProShoppingOutlined',
      title: '商品',
      url: '/system/dict',
      color: '#ff9c6e',
      colProps: { md: 3, sm: 6, xs: 12 }
    },
    {
      id: 4,
      icon: 'IconProLogOutlined',
      title: '订单',
      url: '/system/log/logininfor',
      color: '#b37feb',
      colProps: { md: 3, sm: 6, xs: 12 }
    },
    {
      id: 5,
      icon: 'IconProCopyOutlined',
      title: '票据',
      url: '/monitor/online',
      color: '#ffd666',
      colProps: { md: 3, sm: 6, xs: 12 }
    },
    {
      id: 6,
      icon: 'IconProMailOutlined',
      title: '消息',
      url: '/system/notice',
      color: '#5cdbd3',
      colProps: { md: 3, sm: 6, xs: 12 }
    },
    {
      id: 7,
      icon: 'IconProTagOutlined',
      title: '标签',
      url: '/system/role',
      color: '#ff85c0',
      colProps: { md: 3, sm: 6, xs: 12 }
    },
    {
      id: 8,
      icon: 'IconProControlOutlined',
      title: '配置',
      url: '/system/config',
      color: '#ffc069',
      colProps: { md: 3, sm: 6, xs: 12 }
    }
  ];

  /** 获取缓存的数据 */
  const getCacheData = () => {
    const result = [...DEFAULT];
    try {
      const str = localStorage.getItem(CACHE_KEY);
      const temp = str ? JSON.parse(str) : null;
      if (temp) {
        result.sort((a, b) => temp.indexOf(a.id) - temp.indexOf(b.id));
      }
    } catch (e) {
      console.error(e);
    }
    return result;
  };

  /** 数据 */
  const data = ref(getCacheData());

  /** 重置布局 */
  const reset = () => {
    data.value = [...DEFAULT];
    localStorage.removeItem(CACHE_KEY);
  };

  /** 缓存布局 */
  const cacheData = () => {
    localStorage.setItem(
      CACHE_KEY,
      JSON.stringify(data.value.map((d) => d.id))
    );
  };

  defineExpose({ reset });
</script>

<style lang="scss" scoped>
  .app-link {
    padding: 12px;
    display: block;
    text-align: center;
    text-decoration: none;
    user-select: none;
    color: inherit;

    :deep(.app-link-icon) {
      font-size: 30px;
      color: #69c0ff;
      margin: 6px 0 10px 0;

      & > svg {
        stroke-width: 3;
      }
    }
  }
</style>
