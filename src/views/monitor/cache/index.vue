      ...
      >
        ...
        >
          ...
          <el-descriptions-item label="AOF是否开启">
            <div>{{ data.info?.aof_enabled === '0' ? '否' : '是' }}</div>
          </el-descriptions-item>
          ...
          <el-descriptions-item label="网络入口/出口">
            <div>
              <span>{{ data.info?.instantaneous_input_kbps }}kps</span>
              <span> / </span>
              <span>{{ data.info?.instantaneous_output_kbps }}kps</span>
            </div>
          </el-descriptions-item>
        </el-descriptions>
      </ele-loading>
...
<script setup>
  ...
  useEcharts([commandChartRef, memoryChartRef], true);

  /** 命令统计图表配置 */
  const commandChartOption = ref();
